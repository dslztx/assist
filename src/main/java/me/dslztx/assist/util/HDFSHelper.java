 ...
 * @author dslztx
 * @date 2015年5月21日
 * @description <br> 不使用同一个conf配置实例的原因有两个：<br> 1)重新建立一个新的conf配置实例，能够正常工作；<br>
 * 2)任务在使用conf配置实例的时候，会对其进行一些隐式的设置参数操作，<br> 比如getSnappyCodecSeqFileWriter在使用conf的时候，会设置其压缩参数，这会造成代码间的耦合
 */
//todo:
public class HDFSHelper {
   ...
   */
  public static SequenceFile.Writer getSnappyCodecSeqFileWriter(String uri,
    ...
    try {
      ...
      return SequenceFile.createWriter(conf, SequenceFile.Writer.file(path),
          SequenceFile.Writer.keyClass(outputKeyClz),
          SequenceFile.Writer.valueClass(outputValueClz),
   ...
   * @description <br> 想要读取压缩过的SequenceFile文件，在建立Reader实例的时候，不需要特殊指定压缩参数，<br>
  ...
  public static void main(String[] args) {
    try {
      SequenceFile.Writer writer =
          HDFSHelper
              .getSnappyCodecSeqFileWriter("/dir/stats/20150528", Text.class, IntWritable.class);
